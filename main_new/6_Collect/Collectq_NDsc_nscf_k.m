...
for nisg=1:Nsigma
    ...
    for NK = 1:NKPTS_sym
        ...
        E_up = max([E_up;E_BAND_ALL(1:NBANDS_s,NK)]);
        E_low = min([E_low;E_BAND_ALL(1:NBANDS_s,NK)]);
    end
...
%% Determine all using Gaussian smearing
for NK=1:NKPTS_sym
    ...
    load(['../BASIS_NDsym_new_',int2str(NK),'.mat'])
            ...
            for NK_s=1:length(NK_start_u)
                ...
                for NB = 1:NBEAD/2+1
                    lambda_q{nisg}(NQ,NB) = lambda_q{nisg}(NQ,NB) ...
                        + sum(sum(weight .*...
                        W11_Q1(Band_idx_k,Band_idx_kq,NB)))/BETA/dos_ef(nisg);
                end
                
                for NB = 1:NBEAD1/2+1
                    lambda2_q{nisg}(NQ,NB) = lambda2_q{nisg}(NQ,NB) ...
                        + sum(sum(weight .*...
                        W11_Q2(Band_idx_k,Band_idx_kq,NB)))/BETA/dos_ef(nisg);
                end
            end
        end
        %end
...
save('Result_q_re','rho_2','omega2_2','mustar','Nsigma',...
    ...
    'lambda','lam_no','lambda_q','EFERMI_gauss','dos_ef')
...
Ncut=NBEAD/2;
ns=4;
lam_f=@(x) interp1(BETA*[0:NBEAD/2],lambda2{ns}(1:NBEAD/2+1),x,'spline');
