...
[~,sym_idx]=ismembertol(VKPT_sym.',VKPT_nosym.','ByRows',true);
...
%%
%NK_group = floor(linspace(1,NKPTS+1,NCLUSTER+1));
%NK_BEGIN = NK_group(NCL);
%NK_END = NK_group(NCL+1)-1;

NBANDS_ss = NELE * 2;
E_BAND_ALL_NK = zeros(NBANDS_ss,NKPTS_nosym);

Gbar_inv_all=cell(1,NKPTS_nosym);
for NK=1:NKPTS_sym
    NK_sym=sym_idx(NK);

    [G_INDEX,G,G2,NPL,~,~,~,~]=...
        RD_INDEX(ROOT_DIR,'1','1',...
        VKPT_nosym(:,NK_sym),B,ENCUT);
    ...
    for NS=1:N_SYM_TRUE
        ...
        G_INDEX_o_k = G_INDEX_o_k * VKPT_equiv_factor(NS);
        [~,G_equiv_idx{NS}] = ismembertol(G_INDEX_equiv{NS}',G_INDEX_o_k'...
            ,'ByRows',true);
    end
    
    %%
    %[~,NK_loc] = ismembertol(VKPT_sym(:,NK)',VKPT','ByRows',true);
    
    %load(['Gbar_',int2str(NK_loc),'.mat'],'Gbars')
    load('EFERMI.mat')
    
    %Gbars_o = Gbars;
    unique_NK = unique(VKPT_equiv_idx);
    
    Gbars_uniq=cell(1,N_SYM_TRUE);
    for NS_uniq = 1:length(unique_NK)
       load(['../../main_nscf_new_nosym/Gbar_',int2str(unique_NK(NS_uniq)),'.mat'],'Gbars')
       Gbars_uniq{unique_NK(NS_uniq)} = Gbars;
    end
     
    Gbars_o = zeros(NPL,'single');
    for NS=1:N_SYM_TRUE
        %load(['Gbar_',int2str(VKPT_equiv_idx(NS)),'.mat'],'Gbars')
        
        Gbars_o = Gbars_o + Gbars_uniq{VKPT_equiv_idx(NS)}(G_equiv_idx{NS},G_equiv_idx{NS});
    end
    Gbars = Gbars_o;
    Gbars = Gbars/N_SYM_TRUE;

    
    %%
    if LBloch
        VKPT_NK=(BASIS(NK_sym).VKPT_red.').*BASIS(NK_sym).Factor_sym;
    end
    ...
    if LBloch
        NKPTS_red = BASIS(NK_sym).NKPTS_red;
    else
        %% PAW datasets
        ...
        %% G_INDEX_sc
        NK_sc=1;
        G_INDEX_tmp = G_INDEX_sc_f + VKPT_nosym(:,NK_sym);
    ...
    for NK_s=1:NKPTS_red
        if LBloch
            idx = find(BASIS(NK_sym).Band_kp==NK_s);
            ...
            WAVE{NK_s} = BASIS(NK_sym).WAVE{NK_s}(:,1:NBANDS_s)*BASIS_re;
    ...
    if ~LBloch
        ...
        BASIS(NK_sym).E_BAND=E_BAND;
        BASIS(NK_sym).WAVE=WAVE;
        BASIS(NK_sym).E_BAND_ALL=E_BAND_ALL;
        BASIS(NK_sym).E_idx=E_idx;
        BASIS(NK_sym).Band_idx=Band_idx;
    end

    Gbar_inv = diag(Gi_new);

    Gbar_inv_ori=Gbar_inv;
    BASIS_ori=BASIS;
    
    %%
    clear BASIS
    for NS_uniq = 1:length(unique_NK)
        NK_uniq=unique_NK(NS_uniq);
        [~,NS_idx]=ismember(NK_uniq,VKPT_equiv_idx);
        
        %% G_INDEX_sc
        NK_sc=1;
        G_INDEX_tmp = G_INDEX_sc_f + VKPT_nosym(:,NK_uniq);
        ...
        VKPT_red=zeros(3,NKPTS_red);
        G_kp=cell(1,NKPTS_red);
        WAVE=cell(1,NKPTS_red);
        Factor_sym = ones(NKPTS_red,1);
        VKPTS_idx = zeros(NKPTS_red,1);
        for NK_s=1:NKPTS_red
            G_kp0 = ...
                G_equiv_idx{NS_idx}(BASIS_ori.G_kp{NK_s});
            %ismember(BASIS_ori.G_kp{NK_s},G_equiv_idx{NS_idx});
            
            G_INDEX_sc_sub = ...
                G_INDEX_sc(:,G_kp0);
            
            G_INDEX_sc_pr=mod(N_sc\G_INDEX_sc_sub(1:3,:),1).';
            ...
            VKPTS_idx(NK_s)=VKPT_sps_ia;
            
            VKPT_red(:,NK_s) = VKPT(:,VKPT_sps_ia);
            
            G_INDEX_sym=G_INDEX_prim{VKPT_sps_ia};
            ...
            [~,G_kpidx2] = ismember(G_kp{NK_s},G_kp0);
            WAVE{NK_s}=BASIS_ori.WAVE{NK_s}(G_kpidx2,:);
        end
            
        BASIS.E_BAND=BASIS_ori.E_BAND;
        BASIS.Band_kp=BASIS_ori.Band_kp;
        BASIS.E_BAND_ALL=BASIS_ori.E_BAND_ALL;
        BASIS.E_idx=BASIS_ori.E_idx;
        BASIS.Band_idx=BASIS_ori.Band_idx;
        ...
        Gbar_inv_all{NK_uniq} = Gbar_inv_ori;
        save([path,'/BASIS_NDsym_new_',int2str(NK_uniq),'.mat'],...
            'BASIS','-v7.3')
        
        E_BAND_ALL_NK(:,NK_uniq) ...
            =BASIS.E_BAND_ALL(1:NBANDS_ss);
        
        EFERMI_gauss = EFERMI_Av;
        Gbar_inv = Gbar_inv_all{NK_uniq};
        Gbar_inv = Gbar_inv - EFERMI_Av + EFERMI_gauss;
        Gbar_diag = 1./Gbar_inv;
        
        save([path,'/Gbar_inv_new_',int2str(NK_uniq),'.mat'],...
            'Gbar_diag','Gbar_inv')
